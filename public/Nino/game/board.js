...
class Board extends Nino.Object {
    constructor(properties) {
        ...
        this.materialMatrix = Nino.Math.matrix2d(this.mapWidth, this.mapHeight, 0);
        // Iterate players
            ...
            case "placing":
                ...
                if (Nino.Input.getClickPress("b")) {
                    ...
                    if (this.materialMatrix[yposIndex][xposIndex] != 0) {
                        console.log("No puedes colocar tu personaje sobre un bloque solido");
                        break;
                    }
                    this.lastX[Board.turn] = xposIndex;
                    this.lastY[Board.turn] = yposIndex;
                    Board.players[Board.turn].position = Nino.Math.getPosition(this.xpos, this.ypos);
                    this.turn();
                }
                break;
            case "start":
                if (Nino.Input.getClickPress("b")) {
                    ...
                    for (let position of this.bounds) {
                        if (position.xoff * this.tileSizeX == this.xpos &&
    ...
    draw(ctx, interpolation) {
        ...
        // Dibujar cruceta
        Nino.Load.getSprite("interface-selector")?.draw(ctx, interpolation, Nino.Math.getPosition(this.xpos, this.ypos));
    }
    drawBoard() {
